...
# Define a function to validate intranet IP addresses
validate_intranet() {
  if [ -z "$1" ]; then
  ...
  # Use a regular expression to match reserved intranet IP addresses
  # See https://en.wikipedia.org/wiki/Reserved_IP_addresses
  if [[ "$1" =~ ^(10|172\.(1[6-9]|2[0-9]|3[01])|192\.(168|0\.(0|2)|88\.99)|198\.(1[89]|51\.100)|203\.0\.113|2[23][4-9]|233\.252|(24[0-9]|25[0-5]))\.(25[0-5]|2[0-4][0-9]|[01]?[0-9]?[0-9])\.(25[0-5]|2[0-4][0-9]|[01]?[0-9]?[0-9])\.(25[0-5]|2[0-4][0-9]|[01]?[0-9]?[0-9])$ ]]; then
    return 0
  else
    return 1
  fi
